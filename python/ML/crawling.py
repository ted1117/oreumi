...
if __name__ == "__main__":
    ...
    for url in tqdm(final_urls):
        ...
        if title is None:    # if not title # if title is None # if title == None
            title = news_html.select_one("#content > div.end_ct > div > h2")

        content = news_html.select("div#div_area")
        if not content:
    ...
    news_df = pd.DataFrame({"data": news_dates, "title": news_titles, "link": final_urls, "content": news_contents})
